<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.room-types') }}" class="p-2 rounded-xl bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white transition-all">
                <span class="material-symbols-outlined text-xl">arrow_back</span>
            </a>
            <div>
                <h2 class="text-amber-500 font-black tracking-[0.4em] mb-1 text-[10px] uppercase">Types de Chambres</h2>
                <h3 class="text-4xl font-black tracking-tight text-white">Modifier <span class="text-gradient">{{ $roomType->name }}</span></h3>
            </div>
        </div>
    </x-slot>
